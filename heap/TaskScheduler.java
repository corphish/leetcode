class Solution {
    ...
    // 2. If the task can be executed, execute it (reduce count, update timestamp) and update in the queue.
    // 3. If the task cannot be executed, get the next task in queue that can be executed, execute it and update in queue.
    // 4. Make sure to keep track of the tasks we are checking in step 3, and add them back in queue after a task was executed (this needs
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        for (; !queue.isEmpty(); time++) {
            ...
            if (t.canBeExecuted(time, n)) {
                //System.out.println("Executing " + t + " at " + time);
                int newCount = t.execute(time);
                if (newCount > 0) queue.add(t);
            } else {
                ...
                while (!queue.isEmpty()) {
                    ...
                    if (other.canBeExecuted(time, n)) {
                        //System.out.println("Executing " + other + " at " + time);
                        int newCount = other.execute(time);
                        if (newCount > 0) queue.add(other);
                        break;
                    } else {
                        polled.push(other);
                    }
